...
# Linear velocity setpoint (m/s)
vlin_ref = [0.0, 0.0, 0.0]

# Angular velocity setpoint (rad/s)
vang_ref = [0.0, 0.2, 0.0]

# Square-wave reference frequency (Hz)
# f_ref = 1 / 40.0
...
#K_theta  = 5 * np.diag((1, 1, 1, 1))

# Simulation Time Step
sim_time_step = 0.01

# Simulation Duration
sim_time_end  = 100.0 # seconds
...
motion_model_controller_randomize = 0.05 # 1.0 = 100 percent
np.random.seed(123)
...
#vel_reference = builder.AddSystem(FlippingVectorSource(np.concatenate((vlin_0, vang_0)), np.concatenate((vlin_ref, vang_ref)), f_ref, 0))
vel_reference = builder.AddSystem(ConstantVectorSource(np.concatenate((vlin_ref, vang_ref))))
...
builder.Connect(vel_reference.get_output_port(0), velocity_controller.ref_input_port)
...
workspace = [5, 5, 5]
plot = RigidBodyTrajectoryPlot(state_log.sample_times(), state_log.data().transpose(), input_log.data().transpose(), workspace)
plt.show(block=True)
